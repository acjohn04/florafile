...
export default function ConfirmPage() {
  ...
  if (!plantData) return <div className="p-8 text-center text-on-surface-variant">Loading...</div>;

  const rooms = ["Living Room", "Bedroom", "Office", "Kitchen", "Bathroom", "Balcony", "Hallway"];

  return (
    <div className="max-w-md mx-auto space-y-6">
      <header className="text-center mb-8">
        <h1 className="text-3xl font-heading font-bold text-on-surface">Confirm Details</h1>
        <p className="text-on-surface-variant mt-2">Almost done! Where will this plant live?</p>
      </header>
      ...
      <form onSubmit={handleSave} className="bg-surface-container-low p-6 rounded-3xl border border-surface-container space-y-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-on-surface-variant mb-2">Nickname (optional)</label>
            <input 
              type="text" 
              value={nickname}
              ...
              placeholder="e.g. Robert Planter"
            />
          </div>
        ...
        >
          {isSaving ? (
            ...
            <><Icon name="save" /> Save to Inventory</>
          )}
        </button>
      </form>
    </div>
